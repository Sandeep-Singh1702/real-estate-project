...
st.header("Geo Map(Sector Price per sqft)")
...
fig = px.scatter_mapbox(group_df, lat="latitude", lon="longitude",color="price_per_sqft", size="built_up_area",
                                ...
                                zoom=10, mapbox_style="open-street-map",width=1200,
...
selected_sector = st.selectbox(
    "Select Sector",
    sorted(new_df['sector'].dropna().unique())
)

properties = pd.read_csv("gurgaon_properties.csv")

sector_df = properties[
    properties["address"].str.contains(selected_sector, case=False, na=False)
]
